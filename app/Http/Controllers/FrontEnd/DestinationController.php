...
print date("Ymd H:i:s").'<br>';exit;
    ...
    private function _searchDBs($conn, $keyword){
        ...
        $our_collection = \DB::connection($conn)
            ->table('tb_categories')
            ->select('id')
            ->where('category_alias', 'our-collection')
            ->first();
        ...
        tb_categories.id, 
        tb_categories.parent_category_id, 
        tb_categories.category_name, 
        tb_categories.category_image, 
        tb_categories.category_alias, 
        ptc.category_name as p_name 
        FROM tb_categories 
        LEFT JOIN tb_categories as ptc on ptc.id=tb_categories.parent_category_id 
        WHERE tb_categories.category_published=1 AND 
